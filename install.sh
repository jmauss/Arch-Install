...
laptop_utilities()
{
    arch-chroot /mnt pacman -S iw wpa_supplicant dialog tlp bluez bluez-utils networkmanager mesa xf86-video-intel xf86-input-libinput xorg-server --noconfirm
...
security_tools()
{
    while [ 1 ]; do
        ...
        if [ "$TOOLS" == 'y' ]; then
            ...
            arch-chroot /mnt pacman -S qemu virt-manager ebtables dnsmasq testdisk nmap dnsutils whois openssh postgresql metasploit wireshark-cli john aircrack-ng hashcat hping --noconfirm
